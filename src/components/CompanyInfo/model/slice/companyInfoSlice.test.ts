...
import { CompanyInfoSchema, CompanyReport, Profile, RatiosTTM } from "../types/companyInfo";
import { CompanyInfoReducer } from "./companyInfoSlice";
import { fetchingCompanyInfo } from "../services/fetchingCompanyInfo";

describe("CompanyInfo slice test", () => {
  test("fetchingCompanyInfo pending", () => {
    const state: DeepPartial<CompanyInfoSchema> = {
      isLoading: false,
    };

    expect(
      CompanyInfoReducer(
        state as CompanyInfoSchema,
        fetchingCompanyInfo.pending
      )
    ).toEqual({ isLoading: true });
  });

  test("fetchingCompanyInfo fulfilled", () => {
    ...
    expect(
      CompanyInfoReducer(
        state as CompanyInfoSchema,
        fetchingCompanyInfo.fulfilled(data, "", "")
      )
  ...
  test("fetchingSentimentNews rejected", () => {
    ...
    expect(
      CompanyInfoReducer(
        state as CompanyInfoSchema,
        fetchingCompanyInfo.rejected(new Error("error"), "", '')
      )
