...
import { fetchingCompanyInfo } from "../model/services/fetchingCompanyInfo";
import { CompanyReport, Profile, RatiosTTM } from "../model/types/companyInfo";
import { CompanyInfoActions } from "../model/slice/companyInfoSlice";

describe("CompanyInfo component test", () => {
  ...
  test("Company info render", async () => {
    render(
      <MemoryRouter>
  ...
  test('Company info loading', async() => {
    ...
    await waitFor(() => {
      expect(store.getState().companyInfo.isLoading).toBe(true)
    })
